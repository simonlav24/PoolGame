...
class PoolGame:
    ...
    def main_loop(self):
        ...
        while not done:
            ...
            player_turn_surf = self.font.render(text, True, (255,255,255))
...
def main(*args, **kwargs):
    ...
    cpu_config = {
        Player.PLAYER_1: (kwargs.get('player_one', Player_Type.HUMAN), 3),
        Player.PLAYER_2: (kwargs.get('player_two', Player_Type.CPU), 3),
    }

    game = PoolGame(kwargs.get('rules', Rules.SNOOKER), cpu_config=cpu_config, win=win, clock=clock)
    game.initialize()
    game.main_loop()
